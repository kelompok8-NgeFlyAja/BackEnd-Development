...
const searchFlights = async (req, res) => {
    try {
        const { departureAirportId, arrivalAirportId, departureTime, passengers, seatClasses } = req.query;
        if (isNaN(departureAirportId) || isNaN(arrivalAirportId) || isNaN(passengers) || typeof seatClasses !== "string") {
            return res.status(400).json({ message: "Invalid input data." });
        }

        if (!departureAirportId || !arrivalAirportId || !departureTime || !passengers || !seatClasses) {
        ...
        const availableFlights = flights.filter((flight) => {
            ...
            return availableSeats >= parseInt(passengers); 
        });
